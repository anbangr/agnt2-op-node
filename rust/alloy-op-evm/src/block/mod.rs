...
pub trait SettlementStateDB: Database {
    /// Returns the account's current info together with a best-effort original info snapshot.
...
impl<DB: Database> SettlementStateDB for DB {
    fn settlement_account_infos(
        &mut self,
        address: Address,
    ) -> Result<(revm::state::AccountInfo, revm::state::AccountInfo), Self::Error> {
        let current_info = self.basic(address)?.unwrap_or_default();
        Ok((current_info.clone(), current_info))
    }
}
    ...
    Spec: OpHardforks + Clone,
{
    ...
    pub fn enable_post_exec_verifier(&mut self, payload: PostExecPayload) {
...
mod tests {
    ...
    fn test_finish_reports_all_unconsumed_post_exec_entries() {
        ...
        executor.set_post_exec_mode(SDMExecutionMode::Verify(PostExecPayload {
            version: 1,
