...
impl Encodable for PostExecPayload {
    fn encode(&self, out: &mut dyn BufMut) {
        let list_len = self.version.length() + self.gas_refund_entries.length();
        Header { list: true, payload_length: list_len }.encode(out);
...
impl Decodable for PostExecPayload {
    fn decode(buf: &mut &[u8]) -> alloy_rlp::Result<Self> {
        ...
        let remaining = buf.len();
        let this =
            Self { version: Decodable::decode(buf)?, gas_refund_entries: Decodable::decode(buf)? };
        if buf.len() + header.payload_length != remaining {
            return Err(alloy_rlp::Error::UnexpectedLength);
        }
        Ok(this)
...
/// Synthetic post-execution transaction carrying a [`PostExecPayload`].
#[derive(Debug, Clone, PartialEq, Eq, Hash, Default)]
#[cfg_attr(feature = "arbitrary", derive(arbitrary::Arbitrary))]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[cfg_attr(
...
pub fn build_post_exec_tx(gas_refund_entries: Vec<SDMGasEntry>) -> TxPostExec {
    TxPostExec::new(PostExecPayload { version: 1, gas_refund_entries })
}
